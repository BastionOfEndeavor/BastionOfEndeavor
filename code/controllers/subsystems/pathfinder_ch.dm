...
/proc/pathfinding_run_all(turf/start = get_turf(usr), turf/goal)
	...
	*/
	if(pass_silicons_astar != pass_silicons_jps || pass_silicons_jps  != pass_tg_jps)
		log_and_message_admins("Пара тюрфов [COORD(start)], [COORD(goal)] не нашла тропу алгоритмом AStar: silicons_astar [pass_silicons_astar] silicons-jps [pass_silicons_jps] tg-jps [pass_tg_jps]")
	else
