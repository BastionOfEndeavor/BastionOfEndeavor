...
/proc/add_attack_logs(mob/user, mob/target, what_done, var/admin_notify = TRUE, var/use_async = TRUE) //CHOMPEdit
	...
	if(ismob(user)) //CHOMPEdit Begin
		if(SSdbcore.Connect())
			/* Bastion of Endeavor Translation // Bastion of Endeavor TODO
...
/proc/not_has_ooc_text(mob/user)
	...
	*/
	if (CONFIG_GET(flag/allow_metadata) && (!user.client?.prefs?.metadata || length_char(user.client.prefs.metadata) < 15))
