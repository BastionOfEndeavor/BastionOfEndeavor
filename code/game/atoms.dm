...
/atom/proc/atom_say(message)
	...
	for(var/mob/M in get_mobs_in_view(7, src))
		/* Bastion of Endeavor Translation: // Bastion of Endeavor TODO We've had this set to ncase before but no way to know if that was better
		M.show_message(span_npc_say(span_name("[src]") + " [atom_say_verb], \"[message]\""), 2, null, 1)
		*/
		M.show_message(span_npc_say(span_name("[src]") + " [verb_ru(src, atom_say_verb)], \"[message]\""), 2, null, 1)
		// End of Bastion of Endeavor Translation
		if(M.client)
